...
try {
    ...
    # --- The GitHub release body: this version's CHANGELOG section, in full ---
    $notes = Get-ChangelogSection $version
    if ([string]::IsNullOrWhiteSpace($notes)) { Fail "no CHANGELOG.md section found for [$version]" }

    # Publishing here needs only CHANGELOG.md, but the tag this creates is what F-Droid builds from, and
    # both F-Droid and Play publish the short per-version note from the fastlane tree instead. Worth
    # saying out loud rather than failing over: a missing note harms nobody until those channels are live.
    $notePath = Get-ReleaseNotePath (Get-StelaVersionCode $version)
    $missingNote = $notePath -and -not (Test-Path $notePath)
    ...
    if ($missingNote) {
        Write-Host ''
        Write-Host "  Warning     : $notePath is missing, so F-Droid and Play would show no" -ForegroundColor Yellow
        Write-Host "                release note for $version. Harmless if neither is live yet." -ForegroundColor Yellow
    }
    Write-Host ''
    if (-not $Force) {
