...
2. **The fastlane metadata tree** under `fastlane/metadata/android/en-US/` — **done**, and shared with the
   ...
   five `images/phoneScreenshots/`, `images/icon.png` (512×512, rendered from the adaptive launcher icon —
   F-Droid would otherwise extract one from the APK, but Play requires the file) and
   `images/featureGraphic.png` (1024×500, optional here and required by Play). Both images were rendered
   from the app's own drawables rather than drawn, so they stay in step with the brand colour and glyph if
   either changes.
